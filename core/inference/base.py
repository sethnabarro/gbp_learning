...
from core.factors import UnaryFactor, NonLinearUnaryFactor, AdditiveFilterFactor,\
    AdditiveFilterFactorDecomp, AdditiveFilterFactorSum, AdditiveFilterFactorSumDecomp, \
    PairwiseSmoothingFactorDecomp,AdditiveFilterFactorSumTpose, FeedforwardFilterFactor
from core.utils.utils import tf_fn_wrapper, FixedAttributeFieldsObject, get_n_strides, \
    flatten_filters, get_filter_factor_recon
from core.variables import PixelVariable, FilterVariable, CoeffVariable, BiasVariable
...
class ConvLayer(Layer):
    def __init__(self,
        ...
        self.use_tpose_recon = isinstance(filter_factor, AdditiveFilterFactorSumTpose)
...
class Learner(object):
    ...
    def run_gd_inference(self, learning_rate, optim, *args, **kwargs):
        ...
        for l, layr in enumerate(self.layers):
            ...
            elif hasattr(layr, 'pool_factor'):
                coeffs = variables[l][1]
            elif hasattr(layr, 'upsample_factor'):
                coeffs = variables[l][1]
            elif hasattr(layr, 'dense_factor'):
                coeffs = variables[l][-1]
            elif hasattr(layr, 'pixel_obs_factor'):
                coeffs = variables[l][0]
            elif hasattr(layr, 'segmentation_factor'):
                pass
            elif hasattr(layr, 'softmax_factor'):
                pass
            else:
        ...
        def _inference_iter(itr, state):
            ...
            with tf.GradientTape() as tape:
                ...
                for l, layr in enumerate(self.layers):
                    if isinstance(layr, ConvLayer):
                        ...
                        inps, weights, bias, coeffs = variables[l]
                        kwargs = dict(sum_all=True, coeffs=coeffs)
                        hasgrad[l][0] = not layr.dense_factor.is_noiseless_input
                        hasgrad[l][1] = not layr.fixed_params
                        hasgrad[l][2] = not layr.fixed_params
                        if not layr.dense_factor.is_noiseless_input:
                            ins = inps if ins is None else ins
                            kwargs['inputs'] = ins
                        if not layr.fixed_params:
                        ...
                        inps, = variables[l]
                        ins = inps if ins is None else ins
                        kwargs = dict(sum_all=True, inputs=ins)
                        coeffs = ins
                    elif hasattr(layr, 'segmentation_factor'):
            ...
            # print(vars_with_grads[0][0, 10:15, 10:15])
            # print(grads[0][0, 10:15, 10:15])
    ...
    def run_inference(self, n_iters=100, x_img=None, x_mask=None,
                      maybe_y_label=None, callback_fn=None,
    ...
    def energy(self, sum_over_layers=False, as_numpy=False, *args, **kwargs):
        ...
        if sum_over_layers:
            ...
            for lname, lE in Edict.items():
                if isinstance(lE, tf.Tensor):
                    E_total += tf.reduce_sum(lE)
                elif 'total' in lE:
                    E_total += lE['total']
                else:
                    E_total += sum([E for E in lE.values()])
            Edict['total'] = E_total
