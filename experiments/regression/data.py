...
def load_data(n_tr_points=90, n_te_points=225, seed=411):
    ...
    xs_obs = np.sort(np.random.uniform(-1., 1., size=(90, 1)), axis=0)
    ys_obs = gt(xs_obs) + np.random.normal(size=(n_tr_points, 1)) * 1.5
    ...
    xs_obs_te = np.linspace(-1.05, 1.05, n_te_points)[:, None]
