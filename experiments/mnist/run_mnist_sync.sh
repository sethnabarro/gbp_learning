...
do
  ...
  seed2=$RANDOM
  seed3=$RANDOM
  logdir="${repo_dir}/experiments/mnist/standard_mnist/sync/"
  mkdir -p "${logdir}/"

  cmd="CUDA_VISIBLE_DEVICES='${gpus}' TF_GPU_ALLOCATOR=cuda_malloc_async XLA_FLAGS=--xla_gpu_cuda_data_dir=/rds/general/user/$(whoami)/home/anaconda3/pkgs/cuda-nvcc-12.2.140-0 PYTHONPATH=${PYTHONPATH}:$repo_dir:$repo_dir/experiments/mnist/ $python_exec $repo_dir/experiments/mnist/run.py
    --batchsize-train=200
    --batchsize-test=200
    --n-iters-per-train-batch=800
    --n-iters-per-test-batch=500
    --n-train-eval-breaks=15
